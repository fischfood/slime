 ...
 * Slime Options page
 *
 * This is the main controller for the Slime theme options
...
class Options {
	...
	public function init() {
		if ( false === self::get_theme_options() ) {
			add_option( 'slime_theme_options', self::get_default_theme_options() );
		}

		register_setting( 'slime_options', 'slime_theme_options', array( &$this, 'theme_options_validate' ) );
	}
	...
	public function wp_head() {
		$slime_options = self::get_theme_options();

		if ( ! empty( $slime_options['additional_header_scripts'] ) ) {
			echo wp_kses(
				$slime_options['additional_header_scripts'],
				array(
					'script' => array(
						'src'   => array(),
						'async' => array(),
	...
	public function wp_footer() {
		...
		if ( ! empty( $slime_options['additional_footer_scripts'] ) ) {
			echo wp_kses(
				$slime_options['additional_footer_scripts'],
				array(
					'script' => array(
						'src'   => array(),
						'async' => array(),
	...
	public static function get_default_theme_options() {
		...
		return apply_filters( 'slime_default_theme_options', $default_options );
	}
	...
	public function theme_options_add_page() {
		...
		$theme_page = add_theme_page(
			esc_html__( 'Additional Options', 'slime' ),
			esc_html__( 'Additional Options', 'slime' ),
			'edit_theme_options',
			'theme_options',
	...
	public function theme_options_render_page() {
		...
		<div class="wrap">
				...
				class="nav-tab <?php echo esc_attr( $active_tab_class ); ?>">
					...
					printf(
						esc_html(
							...
							__( '%s Additional Footer Content', 'slime' )
						),
			...
			<form method="post" action="options.php">
				...
				<?php settings_fields( 'slime_options' );
				...
				?>
				<input type="hidden" value="1" name="slime_theme_options[first_run]"/>
				<?php submit_button(); ?>
			</form>
		</div>
	<?php
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( ! empty( $scripts ) ) {
			foreach ( $scripts as $key => $script ) {
				if ( ! empty( $script[0] ) ) {
					...
					wp_register_script( $key, get_stylesheet_directory_uri() . $script[0], $dependencies, SLIME_VERSION, true );
		...
		if ( ! empty( $styles ) ) {
			foreach ( $styles as $key => $style ) {
				if ( ! empty( $style[0] ) ) {
					wp_register_style( $key, get_stylesheet_directory_uri() . $style[0], array(), SLIME_VERSION );
		...
		foreach ( $wp_sidebars as $key => $sidebar ) {
			$sidebars[ 'sidebar_layout_' . $key ] = get_option( 'slime_sidebar_layout_' . $key, '' );
		}
	...
	?>
		<script type="text/javascript">
			...
			var slime = {
				...
				nonce: "<?php echo wp_create_nonce( 'slime-nonce' ); ?>"
			};
			//]]>
		</script>
	<?php
	...
	function theme_options_validate( $input ) {
		...
		return apply_filters( 'slime_theme_options_validate', $output, $input, $defaults );
	}
	...
	public static function get_theme_options() {
		return get_option( 'slime_theme_options', self::get_default_theme_options() );
	}
}
